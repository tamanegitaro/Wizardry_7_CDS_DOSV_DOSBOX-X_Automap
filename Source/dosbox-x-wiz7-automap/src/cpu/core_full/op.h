...
/* Using the instructions.h defines */
	...
	case O_INT:
		...
		if (((inst.entry & 0xFF)==0xcc) && DEBUG_Breakpoint()) 
			...
			return (Bits)debugCallback;
#endif
		// Wizardry Automap: INT3 must be intercepted in the opcode core, not
		// later in CPU_Interrupt(), otherwise the original instruction is skipped
		// or the guest receives a real INT3.
		if ((inst.entry & 0xFF) == 0xcc) {
			if (AutomapBreakpoint()) {
				/* DOSBox-X opcode-core path: the Automap breakpoint handler
				 * restores the original byte and runs the callback immediately.
				 * Continue so the core re-fetches the restored instruction instead
				 * of routing through a DOS callback and seeing stale 0xCC again. */
				continue;
			}
		}
