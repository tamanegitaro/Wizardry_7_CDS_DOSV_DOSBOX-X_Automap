...
/* Wizardry Automap DOSBox-X port: these opcode fragments are included
 * by multiple CPU core translation units, so the automap hook prototypes
 * must live in the fragment itself rather than only in core_normal.cpp or
 * core_full.cpp.
 */
extern bool AutomapBreakpoint(void);
...
#  endif
	...
	CASE_B(0xcc)												/* INT3 */
		...
		if (DEBUG_IntBreakpoint(3))
			return (Bits)debugCallback;
#endif			
		// Wizardry Automap: handle our injected INT3 before guest/debugger sees it.
		// Returning the automap callback is important: the callback loop resumes
		// execution at the restored original opcode and then re-arms breakpoints.
		FillFlags();
		if (AutomapBreakpoint()) {
			/* DOSBox-X opcode-core path: AutomapBreakpoint() restores the
			 * original game byte and runs the Automap callback immediately.
			 * Returning a DOS callback handle here causes DOSBox-X to fetch the
			 * stale 0xCC again at the same CS:IP, producing a second unhandled
			 * INT3 and Wizardry 7 fails with "Unable to Load System Fonts".
			 * Re-fetch the restored original instruction directly instead. */
			continue;
		}
