...
extern void AutomapTerminate(void);
extern void AutoMapDetectGame(char * name, uint16_t loadseg, uint16_t headersize);
extern void AutoMapSetupBreakpoints(void);
...
void DOS_Terminate(uint16_t pspseg,bool tsr,uint8_t exitcode) {
	AutomapTerminate(); // WIZARDY_AUTOMAP_DOSBOXX_PORT_STAGING
...
//			if (readsize!=imagesize) LOG(LOG_EXEC,LOG_NORMAL)("Illegal header");
	...
	DOS_CloseFile(fhandle);
	if (flags != OVERLAY) {
		AutoMapDetectGame(const_cast<char *>(name), loadseg, (uint16_t)headersize);
		AutoMapSetupBreakpoints();
	}
