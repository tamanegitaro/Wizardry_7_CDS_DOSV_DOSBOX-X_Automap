...
{
	CInputBox	*_this;
	_this = (CInputBox *)GetWindowLongPtr(hWnd, GWL_USERDATA);
...
CIB_DECL CIB_CONSTRUCTOR(CIB_OBJ HINSTANCE hInst)
{
	...
	wndInputBox.lpszClassName			= TEXT(CIB_CLASS_NAME);
	...
	this->hwndInputBox = CreateWindow(TEXT(CIB_CLASS_NAME), TEXT(""),
...
CIB_DECL CIB_DESTRUCTOR(CIB_OBJ1)  {
	UnregisterClass(TEXT(CIB_CLASS_NAME), this->hThisInstance);
}
...
void	CIB_DECL create(CIB_OBJ HWND hwndNew)
{
	...
	this->hwndQuery =	CreateWindow(TEXT("Static"), TEXT(""), WS_CHILD | WS_VISIBLE,
							CIB_LEFT_OFFSET, CIB_TOP_OFFSET,
							CIB_WIDTH-CIB_LEFT_OFFSET*2, CIB_BTN_HEIGHT*2,
							this->hwndInputBox, NULL,
							this->hThisInstance, NULL);
	this->hwndEditBox = CreateWindow(TEXT("Edit"), TEXT(""), WS_CHILD | WS_VISIBLE | WS_BORDER | ES_AUTOHSCROLL | ES_LEFT, CIB_LEFT_OFFSET,
							CIB_TOP_OFFSET + CIB_BTN_HEIGHT*2, CIB_WIDTH-CIB_LEFT_OFFSET*3, CIB_BTN_HEIGHT,
							this->hwndInputBox,	NULL,
							this->hThisInstance, NULL);
	this->hwndOk =		CreateWindow(TEXT("Button"), TEXT("OK"), WS_CHILD | WS_VISIBLE | WS_TABSTOP | BS_PUSHBUTTON,
							...
							this->hThisInstance, NULL);
	this->hwndCancel =	CreateWindow(TEXT("Button"), TEXT("Cancel"),
...
void	CIB_DECL show(CIB_OBJ LPCTSTR lpszTitle, LPCTSTR	lpszQuery) {
