...
extern void AutoMapOnCreateDOSFile(const char * fileName);
extern void AutoMapOnOpenDOSFile(const char * fileName, uint8_t flags, uint16_t entry);
...
# endif
	...
	*entry = fcb?handle:psp.FindFreeFileEntry();
	...
	if (foundit) { 
		...
		AutoMapOnCreateDOSFile(fullname); // WIZARDY_AUTOMAP_DOSBOXX_PORT_STAGING
...
# endif
	...
	*entry = fcb?handle:psp.FindFreeFileEntry();
	...
	if (exists || device) { 
		...
		Files[handle]->drive = drive;
		AutoMapOnOpenDOSFile(fullname, flags, *entry); // WIZARDY_AUTOMAP_DOSBOXX_PORT_STAGING
		return true;
	} else {
