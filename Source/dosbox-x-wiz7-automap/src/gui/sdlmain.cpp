    ...
    while (paused) {
        ...
        case SDL_MOUSEMOTION:
            ...
            if (touchscreen_finger_lock == no_finger_id &&
                ...
                AutoMapOnMainWindowMouseMotion(event.motion.x, event.motion.y);
            }
#else
            HandleMouseMotion(&event.motion);
#endif
...
//          || (fullscreen && ((width != currWidth) || (height != currHeight)))
        ...
        if (sdl.window) {
            ...
            static bool automap_created_once = false;
            if (!automap_created_once) {
                automap_created_once = true;
                AutoMapCreate();
# if defined(WIN32) && !defined(HX_DOS)
                AutoMapOnMainWindowCreate((void*)GetHWND());
# endif
            }
...
void GFX_Events() {
   ...
   if(IS_PC98_ARCH) {
    ...
    while (SDL_PollEvent(&event)) {
        if (AutoMapHandleEvent(event)) continue; // WIZARDY_AUTOMAP_DOSBOXX_PORT_STAGING
#if defined(C_SDL2)
...
//                  }
    ...
    AutoMapUpdate(); // WIZARDY_AUTOMAP_DOSBOXX_PORT_STAGING
#else /* SDL 1.x---------------------------------- */
    SDL_Event event;
#if defined (REDUCE_JOYSTICK_POLLING)
